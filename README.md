...
项目源自 [LovType](https://lovtype.com) 已验证的写作与平台排版能力，但作为独立的 MIT 开源项目维护，不是 LovType 全量功能的开源复刻。
...
| 编辑器 | CodeMirror 6、语法高亮、格式工具栏、H1–H3 文章目录、搜索替换、经典预览主题、源码与预览双向滚动同步、复制带格式（粘进公众号 / 飞书 / Word 保留排版）、预览长图下载、导出单文件 HTML、保存 PDF、字数与光标统计 |
| 图文 | 本地图片选择 / 拖入 / 粘贴插入（自动压缩后存进 IndexedDB，导出时内嵌为 data URI）、图片工具条调宽度与左中右对齐、自由比例裁剪；代码块按语言高亮；` ```mermaid ` 画流程图、` ```markmap ` 画思维导图 |
| 小红书 | “内容”编辑器支持文本 / 可编辑 Markdown 预览、发布标题与正文标签快捷复制、7 种常用比例与自定义画布、7 套主题 + 可保存的自定义主题、元素级样式、页码/用户标识/二维码、自动分页、批量 PNG 导出（打包为 ZIP） |
| 公众号 | “内容”编辑器支持文本 / 可编辑 Markdown 预览、“封面”可制作 900×383 横版与 500×500 方形图、7 套主题 + 可保存的自定义主题、8 套标题模板、元素级排版、身份/引导卡片、内联 HTML 复制与下载、文章与双封面发布包 |
...
Markdown 正文是唯一内容源。文档开头可选的 `xhs` Front Matter 只保存小红书标题、发布正文和标签，属于随文件携带的发布元数据；主题、字号、颜色、画布等样式仍保存在浏览器本地，不写回正文或 Front Matter。
...
从本地插入的图片会先缩到 1600px 以内再编码，图片本体收进浏览器的 IndexedDB，正文里只留一条 `fastype-img:…` 的短引用。下载、复制、写回原文件的那一刻再把引用换回内嵌的 data URI——`.md` 拷到哪台机器、传给谁，图都还在，不依赖 FasType 也不依赖图床。

之所以不把 base64 直接写进正文：localStorage 一份配额只有 5 MB 且按 UTF-16 计量，base64 还要再涨三分之一，两三张手机照片就把草稿顶爆；几十万字符的长行也会拖慢编辑器和每 3 秒一次的自动保存。打开的外部 Markdown、升级前存下的旧草稿里如果有内嵌图片，会在启动时自动搬进 IndexedDB。
...
| 数据 | 存在哪 | 是否离开设备 |
| --- | --- | --- |
| Markdown 正文 | 浏览器 localStorage、你选择的本地文件 | 仅在你主动使用 AI 时发送：划词操作发送选中文本及少量上下文，全文操作发送当前正文 |
| 正文插图 | 浏览器 IndexedDB；下载 / 复制出去的 Markdown 里内嵌为 data URI | 否 |
| 头像、公众号封面压缩裁剪图 | 浏览器 IndexedDB；导出的配置文件里内嵌为 data URI | 否 |
| 小红书 / 公众号样式、公众号封面文字与配色 | 浏览器 localStorage | 否 |
...
- **公众号封面原图**：只在当前裁剪窗口使用，不会保存；本地仅保存裁好的 WebP 结果（存在 IndexedDB 里），下载时再生成 900×383 与 500×500 PNG。
- **写回原文件**：依赖 File System Access API。授权后，编辑期间每 3 秒自动写回一次；不支持或未授权时仍会自动保存浏览器本地草稿，并可随时「下载 Markdown」。
- **localStorage**：容量有限（5 MB 上下），现在只放文本——正文、样式、配置，以及指向图片的短引用。写满时会停止自动保存并提示你立刻下载正文，不会覆盖最后一份可读数据。
- **图片**：正文插图、头像、公众号封面裁剪图都存在当前浏览器的 IndexedDB 里，换浏览器、清站点数据都会丢。真正带得走的是下载出来的 Markdown 和导出的配置文件——图片在那里内嵌成 data URI。无痕模式等用不了 IndexedDB 的场景会自动退回内嵌，此时受 localStorage 配额限制。
